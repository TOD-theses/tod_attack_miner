...
from tod_attack_miner.ethutils import section
...
INSIG00 = (
    b"(?:"
    + b"\x60\x00\x60\x00\x52\x60\x04\x60\x00\x60\x1c(?:\x60\x00\x01)?\x37"
    + b"|"
    + PIN
    + b"\x60\x1c\x52"
    + b")"
...
SI2 = re.compile(
    ANYTHING + b"(?:" + PSIG + MLOAD20 + b"|" + MLOAD20 + PSIG + b")" + JMPXX, re.DOTALL
)

# signature on stack, immediately consumed, optionally stored in memory 0x80
ST3 = re.compile(NOTHING, re.DOTALL)
SI3 = re.compile(
    ANYTHING + PINSIG + b"(?:" + DUPMSTORE80 + b")?" + PSIG + JMPXX, re.DOTALL
)
...
SI4 = re.compile(
    ANYTHING + b"(?:" + PSIG + MLOAD00 + b"|" + MLOAD00 + PSIG + b")" + JMPXX, re.DOTALL
)
...
# get input signature from memory address 20, push signature constant and jump on equal or unequal
SI5 = re.compile(
    ANYTHING + b"(?:" + PSIG + MLOAD80 + b"|" + MLOAD80 + PSIG + b")" + JMPXX, re.DOTALL
)
...
SI8 = re.compile(
    b"(?:^|" + ANYTHING + JMPDEST + b")" + PSIG + PINSIG + JMPNE, re.DOTALL
)
        ...
        while result:
            ...
            if result:
                signatures = signatures | {
                    x.rjust(4, b"\x00") for x in result.groups() if x is not None
                }
        if signatures:
            break
